...
function Pharmaceuticals() {
  ...
  // ✅ Normalize function for trimming and casing
  const normalize = (str) =>
    str
      ?.normalize("NFKC")
      .replace(/[\u200B-\u200D\uFEFF\u00A0]/g, "") // removes invisible unicode
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const normalizedSelectedForms = selectedDosageForms.map(normalize).join(',');
      const response = await axios.get(
        `https://mubychem.onrender.com/api/product?page=${currentPage}&limit=${productsPerPage}${normalizedSelectedForms ? `&dosageForm=${normalizedSelectedForms}` : ''}`
      );
      setProducts(response.data.data);
      setTotalPages(response.data.meta.totalPages);
    } catch (error) {
  ...
  useEffect(() => {
    fetchProducts();
  }, [currentPage, selectedDosageForms]);

  useEffect(() => {
    // Frontend filtering based on searchTerm (applied to the products fetched from the backend)
