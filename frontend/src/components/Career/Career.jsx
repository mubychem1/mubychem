...
const JobCard = ({ title, location, description, responsibility, qualification, experience, onOpen }) => (
  <div className="bg-[#ecf2ff] rounded-2xl p-6 flex flex-col items-start w-full max-w-sm border border-gray-400 h-full">
    ...
    <div className="mt-4 flex justify-between w-full">
      {/* Job responsibility Button */}
      <button onClick={() => onOpen(title, description, responsibility, qualification, experience)} 
      className="px-6 py-2 text-yellow-900 border rounded-lg">
        Job Decription
      </button>
      {/* Apply Now Button */}
      <button onClick={() => onOpen()} className="px-6 py-2 bg-[#7B3931] text-white rounded-lg hover:bg-gray-700">
        Apply Now
      </button>
    </div>
  </div>
...
const Career = () => {
  ...
  return (
    <>
      ...
      {/* Job Listing Section */}
      <section className="p-10 bg-[#ecf2ff] min-h-screen" style={{ fontFamily: "Raleway, sans-serif" }}>
      ...
      <Dialog open={isOpen} onClose={() => setIsOpen(false)} className="fixed inset-0 flex items-center justify-center z-50" style={{ fontFamily: "Graphik" }}>
        <div className="absolute inset-0 bg-black opacity-50"></div>
        <div className="bg-white p-6 rounded-lg shadow-xl max-w-xl w-full z-10 max-h-[80vh] overflow-y-auto">
          <div className="mt-4 flex justify-end">
            <button onClick={() => setIsOpen(false)} className="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-700">
            ✖
            </button>
          </div>
          <h3 className="text-xl font-bold text-gray-900 mb-2">{selectedJob.title}</h3>
          <hr />
          <h3 className='text-lg py-2'>Job Overview: </h3>
          <p className=" text-gray-900">{selectedJob.description}</p>
          <h3 className='text-lg py-2'>Essential Duties and Responsibilities:</h3>
          ...
          <h3 className='text-lg py-2'>Salary: No constraint for the right candidate.</h3>
